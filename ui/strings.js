...
const DICT = {
  pl: {
    ...
    'net.exposed': 'W SIECI',
    'net.readOnly': 'tylko odczyt',
    'net.writable': 'Z PRAWEM ZAPISU',
    'net.readOnlyHint': 'Interfejs działa w trybie tylko do odczytu.',
    'net.tipReadOnly': 'Interfejs jest widoczny w sieci lokalnej, w trybie tylko do odczytu. Podglądanie tak, zmiany nie.',
    'net.tipWritable': 'UWAGA: interfejs jest widoczny w sieci lokalnej Z PRAWEM ZAPISU. Każdy, kto zna hasło, może zmienić PIN i usunąć odrzucone QSO.',
    'panel.network': 'Interfejs w sieci',
    'panel.iface': 'Interfejs',
    'panel.willSend': 'Poleci jako',
    'willSend.operator': 'operator',
    'willSend.fromLogger': 'ze znakiem stacji z loggera — żaden cel rozmnażania nie jest włączony',
    'willSend.disabled': 'wyłączonych celów: {n}',
    'willSend.mismatch': 'UWAGA: logujesz jako {logger}, a QSO poleci wyłącznie jako {cele}. Żaden włączony cel nie zapisuje na Twój znak z loggera.',
    'iface.localOnly': 'Tylko na tym komputerze. Zdalnie przez tunel SSH albo po udostępnieniu w sieci (Konfiguracja).',
    'iface.network': 'Wpisz ten adres w przeglądarce na telefonie. Port jest ten sam co lokalnie — HTTPS zmienia tylko schemat adresu.',
    'iface.noPassword': 'Bez hasła — dostępny tylko z tego komputera.',
    'iface.refused': 'Nasłuch w sieci ODRZUCONY —',
    'refuse.brak-hasla': 'nie ustawiono hasła',
    'refuse.tls-wylaczony': 'HTTPS wyłączony w konfiguracji',
    'refuse.brak-certyfikatu': 'nie udało się przygotować certyfikatu',
    'refuse.whatToDo': 'Program został na 127.0.0.1. Sprawdź log — najczęściej to brak praw do katalogu danych albo nieczytelne pliki podane w api.tls.certFile i api.tls.keyFile. Zdalnie bez otwierania portu: tunel SSH.',
    'label.apiHost': 'Kto może otworzyć interfejs',
    'label.apiPassword': 'Hasło do interfejsu',
    'label.apiPort': 'Port interfejsu',
    'confirm.unsaved': 'W konfiguracji są niezapisane zmiany. Co z nimi zrobić?',
    'btn.saveAndLeave': 'Zapisz i wyjdź',
    'btn.discardChanges': 'Porzuć zmiany',
    'btn.stayHere': 'Zostań tutaj',
    'hint.apiPortChanged': 'Po restarcie interfejs będzie pod nowym portem — stary adres w przeglądarce przestanie działać.',
    'hint.apiPortRange': 'Port musi być liczbą z zakresu 1024–65535.',
    'label.apiReadOnly': 'Tylko do odczytu (bez zmian konfiguracji i kolejki)',
    'opt.apiLocal': '127.0.0.1 — tylko ten komputer',
    'opt.apiNetwork': '0.0.0.0 — cała sieć lokalna',
    'note.apiNetwork': 'Udostępnienie w sieci wymaga hasła ORAZ HTTPS. Bez jednego z nich program zostaje na 127.0.0.1 i mówi o tym w logu. Certyfikat wystawia sobie sam, więc przeglądarka raz spyta o zaufanie: chroni to treść na kablu, ale nie przed podszyciem się pod serwer w tej samej sieci.',
    'hint.apiPasswordSet': 'Hasło jest ustawione. Puste pole = bez zmian.',
    'hint.apiPasswordNone': 'Brak hasła. Bez niego udostępnienie w sieci nie zadziała.',
    'hint.apiPasswordShort': 'Hasło musi mieć co najmniej 8 znaków.',
    'hint.apiReadOnlyForced': 'Włączono tryb tylko do odczytu — w sieci to ustawienie domyślne. Odznacz świadomie, jeśli chcesz pozwolić na zmiany.',
    ...
    'panel.statsRange': 'Co pokazujemy',
    'range.today': 'Dziś',
    'range.week': '7 dni',
    'range.month': '30 dni',
    'range.all': 'Wszystko',
    'label.filterRange': 'Zakres',
    ...
    'tray.saveReport': 'Zapisz zgłoszenie…',
    'btn.saveReport': 'Zapisz zgłoszenie do wysłania',
    'hint.reportSaved': 'Zapisano: ',
    'hint.reportSafe': 'PIN-y są zamaskowane, hasła nie ma. Ten plik możesz wysłać — w odróżnieniu od config.json.',
    'hint.reportFailed': 'Nie udało się zapisać zgłoszenia — szczegóły w logu.',
    ...
    'fs.normal': 'tekst normalny',
    'fs.duzy': 'tekst duży',
    'fs.bardzo-duzy': 'tekst bardzo duży',
    ...
    'chk.ok': 'Serwis pozwala kontu {konto} logować na ten znak. Kopie się zapiszą.',
    ...
    'chk.missingStation': 'Konto {konto} nie może logować na {stacja} w tej akcji. '
      + 'Sprawdź, czy stacja jest dodana do akcji i do tego konta.',
    ...
    'confirm.targetsRejected': 'Serwis nie przyjmie kopii dla: {stacje}.\n\nSprawdź, czy '
      + 'te stacje są dodane do akcji i do konta, którego PIN-em lecą kopie.\n\nZapisać '
      + 'mimo to? (jeśli właśnie to poprawiasz na radiodyplom.pl — zapisz, sprawdzenie '
      + 'odświeży się samo)',
    ...
    'hint.fanout': 'Puste = jedno QSO ze znakiem stacji z loggera. Każdy znak musi być '
      + 'dodany do akcji i do konta, którego PIN-em leci kopia — inaczej serwer jej '
      + 'nie zapisze.',
  ...
  en: {
    ...
    'net.exposed': 'ON NETWORK',
    'net.readOnly': 'read-only',
    'net.writable': 'WRITABLE',
    'net.readOnlyHint': 'The interface is running in read-only mode.',
    'net.tipReadOnly': 'The interface is visible on the local network, read-only. Viewing yes, changes no.',
    'net.tipWritable': 'WARNING: the interface is visible on the local network WITH WRITE ACCESS. Anyone who knows the password can change the PIN and delete rejected QSOs.',
    'panel.network': 'Interface on the network',
    'panel.iface': 'Interface',
    'panel.willSend': 'Will be sent as',
    'willSend.operator': 'operator',
    'willSend.fromLogger': 'with the station callsign from the logger — no fan-out target is enabled',
    'willSend.disabled': 'disabled targets: {n}',
    'willSend.mismatch': 'WARNING: you are logging as {logger}, but the QSO will go out only as {cele}. No enabled target records under your logger callsign.',
    'iface.localOnly': 'This computer only. Remotely via an SSH tunnel, or after exposing it on the network (Settings).',
    'iface.network': 'Type this address in the browser on your phone. The port is the same as locally — HTTPS only changes the scheme.',
    'iface.noPassword': 'No password — reachable from this computer only.',
    'iface.refused': 'Network listening REFUSED —',
    'refuse.brak-hasla': 'no password set',
    'refuse.tls-wylaczony': 'HTTPS disabled in the configuration',
    'refuse.brak-certyfikatu': 'could not prepare a certificate',
    'refuse.whatToDo': 'The program stayed on 127.0.0.1. Check the log — usually missing permissions on the data directory, or unreadable files given in api.tls.certFile and api.tls.keyFile. Remotely without opening a port: an SSH tunnel.',
    'label.apiHost': 'Who can open the interface',
    'label.apiPassword': 'Interface password',
    'label.apiPort': 'Interface port',
    'confirm.unsaved': 'There are unsaved changes in the settings. What should happen to them?',
    'btn.saveAndLeave': 'Save and leave',
    'btn.discardChanges': 'Discard changes',
    'btn.stayHere': 'Stay here',
    'hint.apiPortChanged': 'After the restart the interface moves to the new port — the old address in the browser stops working.',
    'hint.apiPortRange': 'The port must be a number between 1024 and 65535.',
    'label.apiReadOnly': 'Read-only (no config or queue changes)',
    'opt.apiLocal': '127.0.0.1 — this computer only',
    'opt.apiNetwork': '0.0.0.0 — the whole local network',
    'note.apiNetwork': 'Exposing on the network requires a password AND HTTPS. Without either, the program stays on 127.0.0.1 and says so in the log. It issues its own certificate, so the browser will ask once whether to trust it: this protects the traffic on the wire, but not against someone impersonating the server on the same network.',
    'hint.apiPasswordSet': 'A password is set. Empty field = no change.',
    'hint.apiPasswordNone': 'No password. Network access will not work without one.',
    'hint.apiPasswordShort': 'The password must be at least 8 characters.',
    'hint.apiReadOnlyForced': 'Read-only mode enabled — that is the default on the network. Untick it deliberately if you want to allow changes.',
    ...
    'panel.statsRange': 'What is shown',
    'range.today': 'Today',
    'range.week': '7 days',
    'range.month': '30 days',
    'range.all': 'All',
    'label.filterRange': 'Range',
    'label.filterOperator': 'Operator',
    ...
    'tray.saveReport': 'Save a report…',
    'btn.saveReport': 'Save a report to send',
    'hint.reportSaved': 'Saved: ',
    'hint.reportSafe': 'PINs are masked and the password is absent. This file is safe to send — unlike config.json.',
    'hint.reportFailed': 'Could not save the report — see the log.',
    ...
    'fs.normal': 'normal text',
    'fs.duzy': 'large text',
    'fs.bardzo-duzy': 'very large text',
    ...
    'chk.ok': 'The service lets account {konto} log for this callsign. Copies will be saved.',
    ...
    'chk.missingStation': 'Account {konto} cannot log for {stacja} in this action. '
      + 'Check that the station is added to the action and to this account.',
    ...
    'confirm.targetsRejected': 'The service will not accept copies for: {stacje}.\n\nCheck '
      + 'that these stations are added to the action and to the account whose PIN sends '
      + 'the copies.\n\nSave anyway? (if you are fixing it on radiodyplom.pl right now — '
      + 'save, the check refreshes itself)',
    ...
    'hint.fanout': 'Empty = one QSO with the station callsign from the logger. Every '
      + 'callsign must be added to the action and to the account whose PIN sends the '
      + 'copy — otherwise the server will not store it.',
